...
public class YebisLib : MonoBehaviour
{
    ...
    private static extern void _Initialize(
        ...
        uint uiInitFlags = (uint)PPFX.EPFX_INITFLAG.PFXINIT_DEPTHFACTORSOURCETEXTURE | (uint)PPFX.EPFX_INITFLAG.PFXINIT_RENDERSCENE_EXTERNAL,
        PPFX.EPFX_APERTURESHAPE eApertureShape = PPFX.EPFX_APERTURESHAPE.PFXAPS_DEFAULT);
#if YEBIS_LIB_GLES
    [DllImport(dll_ES2)]
...
#if YEBIS_LIB_GLES
        if (SystemInfo.graphicsDeviceType == UnityEngine.Rendering.GraphicsDeviceType.OpenGLES2){
            _Initialize_ES2(iWindowWidth, iWindowHeight, iGlareQuality, iDepthOfFieldQuality, uiInitFlags, eApertureShape);
        }else if (SystemInfo.graphicsDeviceType == UnityEngine.Rendering.GraphicsDeviceType.OpenGLES3)
#endif
        {
            _Initialize(iWindowWidth, iWindowHeight, iGlareQuality, iDepthOfFieldQuality, uiInitFlags, eApertureShape);
        }
    }
    ...
    [DllImport(dll)]
    private static extern void _SetTonemapParameters(
        float fExposure = 1.0f,
        float fGamma = 1.0f,
        PPFX.EPFX_TONEMAP eTonemap = PPFX.EPFX_TONEMAP.PFXTM_DEFAULT,
        float fMappingFactor = PPFX.PFX_TONEMAP_MAPPINGFACTOR_DEFAULT,
        float fDitherOffsetScale = PPFX.PFX_DITHERSCALE_AUTO);
#if YEBIS_LIB_GLES
    [DllImport(dll_ES2)]
    private static extern void _SetTonemapParameters_ES2(
        float fExposure = 1.0f,
        float fGamma = 1.0f,
        PPFX.EPFX_TONEMAP eTonemap = PPFX.EPFX_TONEMAP.PFXTM_DEFAULT,
        float fMappingFactor = PPFX.PFX_TONEMAP_MAPPINGFACTOR_DEFAULT,
    ...
    public static void SetTonemapParameters(
        float fExposure = 1.0f,
        float fGamma = 1.0f,
        PPFX.EPFX_TONEMAP eTonemap = PPFX.EPFX_TONEMAP.PFXTM_DEFAULT,
        float fMappingFactor = PPFX.PFX_TONEMAP_MAPPINGFACTOR_DEFAULT,
...
#if YEBIS_LIB_GLES
        if (SystemInfo.graphicsDeviceType == UnityEngine.Rendering.GraphicsDeviceType.OpenGLES2){
            _SetTonemapParameters_ES2(fExposure, fGamma, eTonemap, fMappingFactor, fDitherOffsetScale);
        }else if (SystemInfo.graphicsDeviceType == UnityEngine.Rendering.GraphicsDeviceType.OpenGLES3)
#endif
        {
            _SetTonemapParameters(fExposure, fGamma, eTonemap, fMappingFactor, fDitherOffsetScale);
        }
    }

    //// SetGlareShape
    [DllImport(dll)]
    private static extern void _SetGlareShape(PPFX.EPFX_GLARESHAPE shape);
#if YEBIS_LIB_GLES
    [DllImport(dll_ES2)]
    private static extern void _SetGlareShape_ES2(PPFX.EPFX_GLARESHAPE shape);
#endif

    public static void SetGlareShape(PPFX.EPFX_GLARESHAPE shape)
    {
#if YEBIS_LIB_GLES
        if (SystemInfo.graphicsDeviceType == UnityEngine.Rendering.GraphicsDeviceType.OpenGLES2){
    ...
    private static extern void _SetGlareParameters(
        ...
        PPFX.EPFX_GLAREBRIGHTPASS eGlareBrightPass = PPFX.EPFX_GLAREBRIGHTPASS.PFXGBP_DEFAULT,
...
#if YEBIS_LIB_GLES
        if (SystemInfo.graphicsDeviceType == UnityEngine.Rendering.GraphicsDeviceType.OpenGLES2){
            _SetGlareParameters_ES2(fGlareLuminance, fGlareThreshold, eGlareBrightPass, fGlareRemapFactor);
        }else if (SystemInfo.graphicsDeviceType == UnityEngine.Rendering.GraphicsDeviceType.OpenGLES3)
#endif
        {
            _SetGlareParameters(fGlareLuminance, fGlareThreshold, eGlareBrightPass, fGlareRemapFactor);
        }
    }
    ...
    private static extern void _SetLightShaftParameters(
        ...
        PPFX.EPFX_DEPTHFACTORFORMAT eLightShaftMaskDepthSpace,
...
#if YEBIS_LIB_GLES
        if (SystemInfo.graphicsDeviceType == UnityEngine.Rendering.GraphicsDeviceType.OpenGLES2){
            _SetLightShaftParameters_ES2(fLightShaftScale, fLightShaftLength, fLightShaftGlareRatio, fLightShaftMaskDepthThereshold, eLightShaftMaskDepthSpace, fLightShaftAngleAttenuation, fLightShaftNoiseMask, fLightShaftNoiseFrequency);
        }else if (SystemInfo.graphicsDeviceType == UnityEngine.Rendering.GraphicsDeviceType.OpenGLES3)
#endif
        {
            _SetLightShaftParameters(fLightShaftScale, fLightShaftLength, fLightShaftGlareRatio, fLightShaftMaskDepthThereshold, eLightShaftMaskDepthSpace, fLightShaftAngleAttenuation, fLightShaftNoiseMask, fLightShaftNoiseFrequency);
        }
    }
    ...
    [DllImport(dll)]
    private static extern void _SetVignetteParameters(float fVignette = PPFX.PFX_VIGNETTE_DEFAULT,float fVignetteFovDependence = PPFX.PFX_VIGNETTE_FOVDEPENDENCE_DEFAULT);
#if YEBIS_LIB_GLES
    [DllImport(dll_ES2)]
    private static extern void _SetVignetteParameters_ES2(float fVignette = PPFX.PFX_VIGNETTE_DEFAULT,float fVignetteFovDependence = PPFX.PFX_VIGNETTE_FOVDEPENDENCE_DEFAULT);
#endif

    public static void SetVignetteParameters(float fVignette = PPFX.PFX_VIGNETTE_DEFAULT, float fVignetteFovDependence = PPFX.PFX_VIGNETTE_FOVDEPENDENCE_DEFAULT)
    {
#if YEBIS_LIB_GLES
        if (SystemInfo.graphicsDeviceType == UnityEngine.Rendering.GraphicsDeviceType.OpenGLES2){
    ...
    private static extern void _SetVignetteByOpticsImageCircle(
        float fPowerOfCosineLow = 1.0f,
        float fImageCircle = 2.0f,
        float fPenumbraWidthScale = 1.0f,
        float fImageCircleFovDependence = PPFX.PFX_VIGNETTE_FOVDEPENDENCE_OPTICS_DEFAULT,
        float fPenumbraFovDependence = PPFX.PFX_VIGNETTE_FOVDEPENDENCE_PENUMBRA_DEFAULT,
        bool bIgnoreDepthOfFieldEnable = true,
        bool bApertureOpticalVignette = false);
#if YEBIS_LIB_GLES
    [DllImport(dll_ES2)]
