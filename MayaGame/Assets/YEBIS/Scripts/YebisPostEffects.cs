    ...
    void OnPreRender()
	{
        if (IsEnableYebis())
        {
            ...
            //// YEBIS Setting
            {
                ...
                if (SystemInfo.graphicsDeviceType == UnityEngine.Rendering.GraphicsDeviceType.OpenGLES2)
                {
                    if (enableFXAA) initFlags |= (uint)PPFX.EPFX_INITFLAG.PFXINIT_ANTIALIAS_FLAG;
                }
                else
                {
                    initFlags |= (uint)PPFX.EPFX_INITFLAG.PFXINIT_DEPTHFACTORSOURCETEXTURE;
                    initFlags |= (uint)PPFX.EPFX_INITFLAG.PFXINIT_RENDERSCENE_EXTERNAL;
                    if (enableFXAA) initFlags |= (uint)PPFX.EPFX_INITFLAG.PFXINIT_ANTIALIAS;
                    if (glare.lightShaft.enableLightShaft) initFlags |= (uint)PPFX.EPFX_INITFLAG.PFXINIT_LIGHTSHAFT;
                    if (motionBlur.enableMotionBlur)
                    {
                        initFlags |= (uint)PPFX.EPFX_INITFLAG.PFXINIT_MOTIONBLUR_UNLIMITEDRECURSION;
                        initFlags |= (uint)PPFX.EPFX_INITFLAG.PFXINIT_MOTIONBLUR_INTERNALVELOCITYTEXTURE;
                    }
                    if (lensSimulation.anamorphicLens) initFlags |= (uint)PPFX.EPFX_INITFLAG.PFXINIT_ANAMORPHICLENS;
                    if (sSAO.enableSSAO) initFlags |= (uint)PPFX.EPFX_INITFLAG.PFXINIT_AMBIENTOCCLUSION;
                    if (feedbackEffect.feedbackEffect) initFlags |= (uint)PPFX.EPFX_INITFLAG.PFXINIT_FEEDBACK;
                }

                if (IsUseFloatTextureMode())
                {
                    initFlags |= (uint)PPFX.EPFX_DYNAMICRANGEMODE.PFXDR_HDR_DIRECT;
                }
                else
                {
                    initFlags |= (uint)PPFX.EPFX_DYNAMICRANGEMODE.PFXDR_LDR_REMAP;
                }
                YebisLib.Initialize(yebisDestinationTexture.width, yebisDestinationTexture.height, glare.quality, depthOfField.quality, (uint)initFlags, (PPFX.EPFX_APERTURESHAPE)depthOfField.apertureShape);
                ...
                // グレア
                YebisLib.SetGlareShape((glare.enableGlare ? (PPFX.EPFX_GLARESHAPE)glare.shape : PPFX.EPFX_GLARESHAPE.PFXGS_DISABLE));
                if (glare.enableGlare)
                {
                    YebisLib.SetGlareParameters(glare.luminance, glare.threshold, PPFX.EPFX_GLAREBRIGHTPASS.PFXGBP_DEFAULT, glare.remapFactor);
                    ...
                    if (glare.lightShaft.enableLightShaft)
                    {
                        ...
                        YebisLib.SetLightShaftParameters(
                            ...
                            maskThreshold,
                            PPFX.EPFX_DEPTHFACTORFORMAT.PFXDFFMT_VIEWSPACEDISTANCE,
...
public struct Glare
{
    ...
    public int quality;
    public YebisPostEffects.EPFX_GLARESHAPE shape;
    ...
    public void Reset()
    {
        enableGlare = true;
        quality = 3;
        shape = YebisPostEffects.EPFX_GLARESHAPE.PFXGS_BLOOM;
        luminance = 1.0f;
        threshold = 0.0f;
        remapFactor = 1.0f;
        lightShaft.Reset();
...
public struct DepthOfField
{
    ...
    public int quality;
    public YebisPostEffects.EPFX_APERTURESHAPE apertureShape;
    ...
    public void Reset()
    {
        enableDof = true;
        quality = 5;
        apertureShape = YebisPostEffects.EPFX_APERTURESHAPE.PFXAPS_AUTO;
        filmSize = 0.24f;
...
public struct LensSimulation
{
    ...
    public void Reset()
    {
        ...
        //vignetteFovDependence = PPFX.PFX_VIGNETTE_FOVDEPENDENCE_DEFAULT;
        powerOfCosine = 1.0f;
        imageCircle = 2.0f;
        penumbraWidth = 1.0f;
        //penumbraFovDependence = PPFX.PFX_VIGNETTE_FOVDEPENDENCE_PENUMBRA_DEFAULT;
